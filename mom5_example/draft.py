...
data = (
    data["usurf"]
    .isel(time=0)
    .sel(xu_ocean=slice(-100, -0), yu_ocean=slice(-50, 50))
    .compute()
)
...
grid_data = grid_data.interp(dict(xt_ocean=data.xu_ocean, yt_ocean=data.yu_ocean))

# Still not sure about the parameter dxmin
cartesian_filter = filter.Filter(
    filter_scale,
    dx_min=1,
    n_steps=10,
